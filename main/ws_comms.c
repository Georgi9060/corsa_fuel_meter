...
extern char currently_open_page[30];

const char *TAG = "ws_comms";

void send_comms_data_pack(comms_data_pack_t data) {
    char buf[128];
    snprintf(buf, sizeof(buf), "c|%d|%d|%d|%d|%d|%d|%d|%d|%d",
                                data.load,
                                data.coolant_temp,
                                data.rpm,
                                data.speed,
...
void send_debug_fuel_data_pack(debug_fuel_data_pack_t data) {
    char buf[128];
    snprintf(buf, sizeof(buf), "d|%.1f|%.1f|%d|%.2f|%d|%d|%d|%d|%.0f|%.1f",
                     data.inst_fuel,
                     data.avg_fuel,
                     data.coolant_temp,
                     data.cons_fuel,
...
void send_fuel_data_pack(fuel_data_pack_t data) {
    char buf[128];
    snprintf(buf, sizeof(buf), "f|%.1f|%.1f|%d|%.2f",
                                data.inst_fuel,
                                data.avg_fuel,
                                data.coolant_temp,
                                data.cons_fuel
